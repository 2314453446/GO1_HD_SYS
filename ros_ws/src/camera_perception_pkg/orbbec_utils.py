...
# orbbec 相机rgb像素点坐标->相机空间座标点转换
def rgb_pixels_to_color_camera_xyz_batch_from_frames(color_frame, depth_frame, points_uv, win=3, out_in_meter=True):
    """
    points_uv: [(u1,v1), (u2,v2), ...]  # RGB图像像素坐标
    return: [(X,Y,Z) or None, ...]      # 无效深度返回 None
    """
    ...
    depth_scale = depth_frame.get_depth_scale()  # usually mm scale
    ...
    for (u, v) in points_uv:
        ...
        if out_in_meter:
            results.append((x / 1000.0, y / 1000.0, z / 1000.0))
        else:
            results.append((x, y, z))  # mm
