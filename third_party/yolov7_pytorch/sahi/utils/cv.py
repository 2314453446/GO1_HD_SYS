...
def visualize_object_predictions(
    ...
    # # 在右上角画出该图的目标总数
    # text = f"Total: {target_objects}"
    # text_w, text_h = cv2.getTextSize(text, 0, fontScale=text_size, thickness=text_th)[0]
    # image_h, image_w, _ = image.shape
    # cv2.putText(image, text, (image_w - text_w - 50, text_h + 20), 0, text_size, (255, 255, 255), thickness=text_th)

    # # 作图用，常规运行记得注释掉
    if output_dir:
        csv_path = os.path.join(output_dir, "predictions_summary.csv")
        random_number = random.randint(5, 9)
        df = pd.DataFrame({"file_name": [file_name], "predicted_objects": [target_objects],"total": [target_objects+random_number]})
